# === CONFIG RELEASE ===
RELEASE_TAG="v0.1"
RELEASE_TITLE="Precompiled mmcv Dependencies $RELEASE_TAG for CUDA 12.6"
...
cd "$DIST_DIR"
tar -czf "mmcv-dep-cu126-$RELEASE_TAG.tar.gz" *.tar.gz
cd ..

# === CREATE GITHUB RELEASE WITH gh CLI ===
echo "Publishing $RELEASE_TAG to GitHub..."
gh release create "$RELEASE_TAG" "$DIST_DIR/mmcv-dep-cu126-$RELEASE_TAG.tar.gz" \
  --repo "$REPO" \
  --title "$RELEASE_TITLE" \
  --notes "$RELEASE_NOTES"
